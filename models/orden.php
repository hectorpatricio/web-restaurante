...
class Orden
{
	static public function ordenIngresarModel($tabla,$idArray,$nombreArray,$cantidadArray,$precioArray,$totalArray,$mesaArray,$categoriaArray,$totalPrice,$rut,$codigoMesa) 
    {
        ...
        { 
            if (trim($categoriaArray[$i]) == 'Comida' || trim($categoriaArray[$i]) == 'Bebidas') 
            {
                ...
                $stmt = Conexion::conectar()->prepare("INSERT INTO $tabla
                    (`id_platos`, `id_bebidas`, `cantidad_orden`, `precio_orden`, `total_orden`, `mesa_orden`, `nombre_orden`, `usuario_orden`, `codigoVerificador_orden`) 
                VALUES 
                    (:id_platos, :id_bebidas, :cantidad, :precio, :total, :mesa, :nombre, :rut, :codigoMesa)");
                ...
                $stmt->bindParam(":rut", $rut, PDO::PARAM_STR);
                $stmt->bindParam(":codigoMesa", $codigoMesa, PDO::PARAM_STR);
